...
namespace lbfgs
{
    ...
    struct lbfgs_parameter_t
    {
         ...
         */
        /* 近似逆 hessian 矩阵的校正次数。*/
        /* 存储先前m次迭代的计算结果，以逼近当前迭代的逆 hessian 矩阵。此参数控制有限内存的大小*（更正）。默认值为 8。* 不推荐小于 3 的值。较大的值会导致计算时间过长。*/
        int mem_size = 8;
        ...
        /* 此参数确定要找到的解决方案的精度。当||g(x)||_inf / max(1, ||x||_inf) < g_epsilon, * 时，最小化终止，其中 ||.||_inf 是无穷范数。默认值为 1.0e-5。*/
         ...
         */
        /* * 基于增量的收敛测试的距离。*/
        /* 此参数确定迭代中的距离，以计算成本函数的下降率。如果此参数的值为零，则库不执行基于 delta 的收敛测试。默认值为 3。*/
        int past = 3;
         ...
         */
        // Delta 用于收敛测试。
        // 此参数确定成本函数的最小下降率。当满足以下条件时，库将停止迭代：|f' - f| /max(1, |f|) < delta，其中 f' 是过去迭代之前的成本值，f 是 * 当前迭代的成本值。默认值为 1.0e-6。
        double delta = 1.0e-3;
         ...
         */
        // 最大迭代次数。
        // 当迭代计数超过此参数时，lbfgs_optimize() 函数会以 * ::LBFGSERR_MAXIMUMITERATION 状态代码终止最小化过程。将此参数设置为零会继续最小化过程，直到收敛或错误。默认值为 0。
                                        ...
                                        const lbfgs_parameter_t &param)
    {
        ...
        while (true)
        {
            ...
            else
            {
                ...
                if (g.dot(s) < dstest)
                {
                    // std::cout<<"Wolfe condition not satisfied."<<std::endl;
                    mu = stp;
                }
                else
                {
